...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "jbpm-utils-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name jbpm-utils-service apps/jbpm-utils-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app jbpm-utils-service:1.0-SNAPSHOT
    oc expose svc/jbpm-utils-service
else

	echo "Launching the application locally..."
	pattern="jbpm-utils-service"
	files=( $pattern )
	cd ${files[0]}
